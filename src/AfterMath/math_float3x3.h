...
namespace AfterMath
{
    ...
    /*inline float3x3::float3x3(const float4x4& mat4x4) noexcept
        : row0(mat4x4[0].xyz()), row1(mat4x4[1].xyz()), row2(mat4x4[2].xyz()) {}
    ...
    inline float3x3::float3x3(const quaternion& q) noexcept {
        ...
        row0 = float3(1.0f - 2.0f * (yy + zz), 2.0f * (xy - wz), 2.0f * (xz + wy));
        row1 = float3(2.0f * (xy + wz), 1.0f - 2.0f * (xx + zz), 2.0f * (yz - wx));
        row2 = float3(2.0f * (xz - wy), 2.0f * (yz + wx), 1.0f - 2.0f * (xx + yy));
    }
