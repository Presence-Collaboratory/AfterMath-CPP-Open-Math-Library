...
namespace AfterMath
{
    ...
    inline half4 unpremultiply_alpha(const half4& color) noexcept {
        ...
        if (std::abs(fw) < 1e-6f) {
            ...
            return half4(
                half(std::clamp(float(color.x) * inv_alpha, -1000.0f, 1000.0f)),
                half(std::clamp(float(color.y) * inv_alpha, -1000.0f, 1000.0f)),
                half(std::clamp(float(color.z) * inv_alpha, -1000.0f, 1000.0f)),
                color.w
            );
        }
    ...
    inline const half4 half4_White(half_One);
